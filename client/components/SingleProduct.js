...
class SingleProduct extends React.Component {
  ...
  async componentDidMount() {
    try {
      ...
      console.log(
        "error in componentDidMount of SingleProduct component: ",
        err
      );
    }
  }
  ...
  handleChange(event) {
    if (Number(event.target.value) > this.props.product.quantity) {
      ...
      this.setState({
        ...
        selectQuantity: String(this.props.product.quantity),
      });
    } else {
      this.setState({
        quantity: Number(event.target.value),
        selectQuantity: event.target.value,
      });
    }
  }
...
const mapDispatch = (dispatch, { history }) => {
  return {
    ...
    decreaseStock: (productId, decrement) =>
      dispatch(decreaseStock(productId, decrement)),
  };
};
